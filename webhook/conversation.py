...
- If add_band returns status=ambiguous, don't guess and don't add anything yet — ask the user which of the candidates they mean (or if none match), using its "question" and each candidate's "description" so they can tell the acts apart, then call add_band again with their answer.
